...
layui.define(['jquery', 'form'],
    function (exports) {
        ...
        */
        ...
        EnhanceForm.prototype.setSelectVal_fromId = function (id, val, isOnSelect) {
            if (name === undefined) {
                throw "name no undefined";
            }
            formObj.find(id).val(val);
            form.render('select');
            if (typeof (isOnSelect) === "boolean") {
                if (isOnSelect) {
                    formObj.find("dd[lay-value='" + val + "']").trigger("click");
        ...
        */
            ...
            for (var key in data) {
                if (data.hasOwnProperty(key)) {
                    ...
                    if (inputs.length > 0) {
                        ...
                        if (select.length > 0) {
                            this.setSelectVal(key, data[key], false);
                            //console.log(key +"-"+ data[key]);
                        }
                        else {
                            //对应textarea情况
                            var textAreaObjs = formObj.find('[name="' + key + '"]');
                            if (textAreaObjs.length > 0) {
                                var input = textAreaObjs[0];
                                switch (input.type) {
                                    case "textarea":
                                        input.value = data[key];
                                        break;
        ...
        EnhanceForm.prototype.filling_fromId = function (data) {
            if (typeof data !== "object") {
                throw "data no object";
            }
                ...
                if (data.hasOwnProperty(key)) {
                    ...
                    if (inputs.length > 0) {
                        ...
                        switch (input.type) {
                            ...
                            case "select-one":
                                //this.setSelectVal(key, data[key], false);
                                var select = formObj.find(vKey);
                                if (select.length > 0) {
                                    this.setSelectVal_fromId(vKey, data[key], false);
                                }
                                break;
                            case "hidden":
                                input.value = data[key];
