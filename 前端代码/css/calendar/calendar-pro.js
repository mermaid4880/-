(function () {
    ...
    function getIndexDay() {
        ...
        newtoday = newyear + "-" + newmonth + "-" + newdate;
        ...
        for (var i = calendarDate.monthStart; i > 0; i--) {
           ...
           // var dataDateStr = calendarDate.lastYear + "-" + calendarDate.lastMonth + "-" + (calendarDate.lastDays - i + 1);//去掉每月之前的那几天
            var dataDateStr = "";
            
            riliHtml += '<div class="ht-rili-td ht-rili-td" data-date="' + dataDateStr + '"><span class="ht-rili-day">  &nbsp;   </span><span class="ht-rili-money"></span></div>'
        }
       console.log(calendarDate.opt.data);
        for (var k = 0; k < calendarDate.days; k++) {
            ...
            for (var d in calendarDate.opt.data) {
                ...
                if (flag) {
                    ...
                    if (nowDate == newtoday) {
                        ...
                        riliHtml += '<div class="ht-rili-td ht-rili-onclick" style="background:#009688" data-date="' + dataDateStr + '"><span class="ht-rili-day">' + (k + 1) + '</span><span class="ht-rili-money" data-money="' + calendarDate.opt.data[d].data + '">' + calendarDate.opt.data[d].data + '</span></div>';
                    } else { 
            ...
            if (!flag) {
                riliHtml += '<div class="ht-rili-td ht-rili-td" data-date="' + dataDateStr + '"><span class="ht-rili-day">' + (k + 1) + '</span><span class="ht-rili-money"></span></div>';
            }
        }
        for (var j = 0; j < (42 - calendarDate.days - calendarDate.monthStart); j++) {
            var dataDateStr = calendarDate.nextYear + "-" + calendarDate.nextMonth + "-" + (j + 1);//当前月30号之后的下一月的

           
            //riliHtml += '<div class="ht-rili-td ht-rili-td" data-date="' + dataDateStr + '"><span class="ht-rili-day">' + (j + 1) + '</span><span class="ht-rili-money"></span></div>';
        }
