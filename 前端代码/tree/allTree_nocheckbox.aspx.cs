...
public partial class tree_allTree_nocheckbox : System.Web.UI.Page
{
    ...
    List<MeterPropertyCls2> l_MeterProperty = new List<MeterPropertyCls2>();
        ...
        if (ja.Count > 0)
        {
            for (int i = 0; i < ja.Count; i++)
            {
                try
                {
                    ...
                    //这里把点位的信息 保存到数组里面
                    MeterPropertyCls2 mpc = new MeterPropertyCls2();
