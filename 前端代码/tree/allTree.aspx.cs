...
public partial class tree_allTree : System.Web.UI.Page
{
    ...
    List<MeterPropertyCls> l_MeterProperty = new List<MeterPropertyCls>();
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        string url = PublicMethod.Public_GetWebConfigStr("interFaceUrl") + "/areas/tree";
        ...
        if (ja.Count > 0)
        {
            for (int i = 0; i < ja.Count; i++)
            {
                try
                {
                    ...
                    //这里把点位的信息 保存到数组里面
                    MeterPropertyCls mpc = new MeterPropertyCls();
                    ...
                    mpc.areaId = o["areaId"].ToString();//新增一个所属区域
                    mpc.deviceId = o["deviceId"].ToString();//新增一个所属设备id
